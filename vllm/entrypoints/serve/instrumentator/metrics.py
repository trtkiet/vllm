...
# --- prometheus-fastapi-instrumentator / FastAPI >= 0.137 compatibility ---
# Instrumentator's default `_get_route_name` requires every matched route to
# expose `.path`, but FastAPI 0.137 introduced `_IncludedRouter` containers
# that match requests while only exposing the selected child route through
# their private matcher. Without the patch below, `/metrics` requests crash
# with an AttributeError. Remove this whole block once the pinned
# prometheus-fastapi-instrumentator release handles those routers itself.
...
def _patch_instrumentator_routing() -> None:
    """Swap in a route-name resolver that tolerates FastAPI router containers.

    Idempotent; see the compatibility note above `_get_route_path`.
    """
